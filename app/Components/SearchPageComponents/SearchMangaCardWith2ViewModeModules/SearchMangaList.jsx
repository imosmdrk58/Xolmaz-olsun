...
function SearchMangaList({
  ...
  return (
    <article
      className="relative mt-2 tracking-wide bg-gradient-to-br from-slate-950 to-slate-900 rounded-xl overflow-hidden border border-slate-800 cursor-pointer group hover:border-violet-500/50 hover:shadow-md hover:shadow-violet-500/20 group-hover:-translate-y-0.5 transition-transform"
    ...
    >
      <div className="flex flex-col sm:flex-row">
        ...
        <div className="relative w-full sm:w-28 md:w-32 h-48 sm:h-40 md:h-40 flex-shrink-0">
          <Image
            src={manga.coverImageUrl || "/placeholder.jpg"}
            width={136}
            height={204}
            alt={`${manga.title} cover`}
            className="object-cover w-full h-full rounded-t-xl sm:rounded-l-xl sm:rounded-tr-none transition-transform group-hover:scale-[102%]"
            loading="lazy"
            priority={false}
          />
          <StableFlag
            code={manga.originalLanguage || "UN"}
            className="h-auto w-4 absolute bottom-1.5 right-4 shadow-black shadow-md"
          />

          {/* Content Rating Badge */}
          <div className="absolute top-0 left-1 z-10">
            <span
              className={`inline-flex items-center px-2 py-0.5 rounded-md text-[10px] sm:text-xs font-medium backdrop-blur-md shadow-md ${
                {
        ...
        <div className="flex-1 p-4 relative overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-1.5">
            ...
            <div className="flex flex-wrap sm:flex-nowrap items-center gap-2 sm:gap-4">
              ...
              >
                <span
                  className={`relative flex h-2 w-2 rounded-lg ${
                    {
              ...
              {/* Update Time */}
              <span className="text-slate-500 tracking-widest md:mr-20 lg:mr-0 text-xs whitespace-nowrap">
                Updated {timeSinceUpdate(manga.updatedAt)}
              </span>

              {/* Rating */}
              {manga.rating?.rating?.bayesian > 0 && (
                <div className="bg-slate-800/70 -mt-4 md:absolute lg:relative md:top-2 md:-right-6 lg:right-0 md:bg-transparent lg:bg-slate-800/70 md:flex-col lg:flex-row backdrop-blur-sm md:backdrop-blur-none lg:backdrop-blur-sm rounded-lg p-2 px-2.5 flex items-center gap-1.5 shadow-inner">
                  <span className="text-amber-400 font-bold text-base leading-none">
                    {manga.rating.rating.bayesian.toFixed(1)}
                  </span>
                  <div className="transform md:bg-slate-800 lg:bg-transparent rounded-lg md:p-2 lg:p-0 md:rotate-90 md:transform md:translate-y-10 lg:translate-y-0 lg:rotate-0">
          ...
          <div className="mb-1.5">
            ...
            <p className="text-xs mt-1.5 text-white flex flex-wrap items-center gap-1.5">
          ...
          {/* Description */}
          <p className="text-xs tracking-wide text-slate-400 mb-0.5 leading-relaxed line-clamp-2 max-w-full sm:max-w-[85%]">
