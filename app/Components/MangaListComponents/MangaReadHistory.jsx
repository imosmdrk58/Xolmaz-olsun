...
function MangaReadHistory() {
    ...
    return (
        <div className="w-[100% -12px] ml-2 px-6 mb-6">
            {/* Header */}
            <div className="flex mx-1 mb-7 items-center justify-between">
                <div className="flex items-center gap-3">
                    <div className="bg-white/10 p-3 rounded-lg">< BookOpen className={`w-6 h-6 text-purple-400   drop-shadow-md`} /></div>
                    <div>
                        <h2 className="text-lg font-semibold text-white">READ HISTORY</h2>
                        {readHistory.length > 0 && (<p className="text-xs text-gray-400  tracking-wide">
            ...
            <div className="space-y-4">
                {readHistory.length === 0 ? (
                    <div className="flex flex-col items-center ml-2 mt-11 justify-center py-16 px-4 bg-gradient-to-br from-gray-900/50 to-gray-800/30 rounded-xl  backdrop-blur-sm relative overflow-hidden">
                        {/* Background decoration */}

                        <div className="relative z-10 flex flex-col items-center">
                            <div className="w-20 h-20 bg-gradient-to-br from-purple-500/20 to-blue-500/20 rounded-full flex items-center justify-center mb-6  shadow-lg">
                    ...
                    <>
                        <div className="space-y-3">
                            {readHistory.sort((item1, item2) => new Date(item2.lastReadAT) - new Date(item1.lastReadAT)).slice(0, shownMangasInHistory).map((item, index) => {
                                ...
                                return (
                                    <div
                                        key={item.manga.id}
                                        className="group relative  rounded-xl  backdrop-blur-sm overflow-hidden"
                                        style={{
                        ...
                        {readHistory.length > 2 && (
                            <button
                                ...
                                className="w-full flex items-center justify-center gap-2 py-2.5 text-sm font-medium text-gray-400 hover:text-gray-300 hover:bg-gray-800/30 rounded-lg transition-all duration-200"
                            >
                                {isExpanded ? (
                                    <>
